...
export const useSearchAllStocks = (search: string, opts?: { enabled?: boolean }) => {
    return useInfiniteQuery({
        queryKey: ['search-all', search],
        enabled: !!search && (opts?.enabled ?? true),
        queryFn: async ({ pageParam = 1 }) => {
            const url = new URL('https://data-api.agentos.cloud/noodle/search-all-stocks');
            url.searchParams.set('q', search);
            url.searchParams.set('page', String(pageParam ?? 1));
            url.searchParams.set('limit', '20');
...
export const useSearchAllCommodities = (search: string, opts?: { enabled?: boolean }) => {
    return useInfiniteQuery({
        queryKey: ['search-all', search],
        enabled: !!search && (opts?.enabled ?? true),
        queryFn: async ({ pageParam = 1 }) => {
            const url = new URL('https://data-api.agentos.cloud/noodle/search-all-commodities');
            url.searchParams.set('q', search);
            url.searchParams.set('page', String(pageParam ?? 1));
            url.searchParams.set('limit', '20');
