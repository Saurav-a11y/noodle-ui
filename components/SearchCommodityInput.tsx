...
import { Panda, Search, TrendingUpDown, Zap } from "lucide-react"
import { Input } from "./ui/Input"
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import Link from "next/link";
import clsx from "clsx";
import { formatCurrency } from "@/lib/format";
import { useSearchAllCommodities } from "@/hooks/useSearchAll";
...
const SearchCommodityInput = ({
	...
	return (
		<div
			ref={wrapperRef}
			className={`flex-1 max-w-[200px] mx-8 hidden md:block ${inputClassname}`}>
					...
					>
						{isLoading ? (
							...
							<>
								{flat.map((item: any) => (
									<Link
										href={`/commodities/${item.name_slug}`}
										key={item.name}
										className={clsx(
...
export default SearchCommodityInput