		...
		<div className="text-[#1E1B39">
			<div className="rounded-xl p-4 mb-4 bg-[var(--bg-block)] text-[var(--text)]">
				...
				{isFetching ? (
					...
					<div className="flex items-center gap-2">
						<p className="text-3xl font-semibold font-noto">{stockOverview?.health_score?.value}</p>
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
				<div className="rounded-xl p-4 space-y-1 bg-[var(--bg-block)] text-[var(--text)]">
					...
					{isFetching ? <div className="h-5 w-full bg-[var(--loading)] rounded animate-pulse" /> : <div className="text-sm font-medium font-noto flex items-center gap-1" style={{ color: stockOverview?.health_metrics?.authentic_engagement?.color }}>{stockOverview?.health_metrics?.authentic_engagement?.value}</div>}
				</div>
				<div className="rounded-xl p-4 space-y-1 bg-[var(--bg-block)] text-[var(--text)]">
					<CommunityGrowthIcon />
					<div className="flex items-center gap-2 mb-2">
					...
					{isFetching ? <div className="h-5 w-full bg-[var(--loading)] rounded animate-pulse" /> : <div className="text-sm font-medium font-noto flex items-center gap-1">{stockOverview?.health_metrics?.community_growth}</div>}
				</div>
				<div className="rounded-xl p-4 space-y-1 bg-[var(--bg-block)] text-[var(--text)]">
					...
					{isFetching ? <div className="h-5 w-full bg-[var(--loading)] rounded animate-pulse" /> : <div className="text-sm font-medium font-noto flex items-center gap-1">{stockOverview?.health_metrics?.recent_activity_drop}</div>}
				</div>
			</div>
		</div>
	)
