...
const AddToCommodityWatchlist = () => {
	...
	const { data: commodityOverviewData } = useCommodityOverview(code);
					...
					onSuccess: () => {
						if (data?.data?.id) {
							addLog({
								...
								assetLogo: '',
								content: `Added ${commodityOverview.name} (${commodityOverview.symbol}) to watchlist`,
							});
						}
						queryClient.invalidateQueries({ queryKey: ['watchlist', data?.data?.id] });
						queryClient.invalidateQueries({
...
export default AddToCommodityWatchlist