...
const AgriculturalCommoditiesOverview = ({ data, isLoading }) => {
				...
				<Table>
					...
					<TableBody>
						{isLoading
							...
							: _map(data, (agricultural, index) => (
								<TableRow
									key={index}
									className="hover:bg-[#F9F9F9] dark:hover:bg-[#1A1A1A] cursor-pointer transition-colors"
									onClick={() => router.push(`/cryptocurrencies/${agricultural?.name_slug}`)}
								>
									...
									<TableCell className="border-b border-b-[#F3F3F3] dark:border-b-[#242424]">
										<div className="flex items-center gap-3">
											<div className="w-8 h-8 flex items-center justify-center font-noto">
												<HerbsIcon />
												{/* <Image src={_get(agricultural, 'medium_logo_url', '/images/icon-section-6_2.png')} alt="Symbol" width={64} height={64} className="rounded-full" /> */}
											</div>
											<div className="text-[#4B4A4A] dark:text-[#FFF]">
												<p className="font-medium text-sm font-noto">{agricultural?.name}</p>
												<div className="text-[10px] font-medium opacity-50 font-noto">{agricultural?.unit}</div>
											</div>
										</div>
									</TableCell>
									<TableCell className="font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto"><p className="text-center">{formatNumberShort(agricultural?.healthScore)}</p></TableCell>
									<TableCell className="font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto"><p className="text-center">{formatNumberShort(agricultural?.price)}</p></TableCell>
									<TableCell className="font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto"><div>{agricultural?.day}</div></TableCell>
									<TableCell className="font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">
										<div className={`text-center font-noto space-x-1 ${agricultural?.trend === 'up' && 'text-[#00B552]'} ${agricultural?.trend === 'down' && 'text-[#FF0000]'}`}>
											<span>{agricultural?.trend === 'up' && '▲'}</span>
											<span>{agricultural?.trend === 'down' && '▼'}</span>
											<span>{agricultural?.percent}</span>
										</div>
									</TableCell>
									<TableCell className="text-sm font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">{agricultural?.weekly}</TableCell>
									<TableCell className="text-sm font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">{agricultural?.monthly}</TableCell>
									<TableCell className="text-sm font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">{agricultural?.ytd}</TableCell>
									<TableCell className="text-sm font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">{agricultural?.yoy}</TableCell>
									<TableCell className="text-sm font-medium text-[#4B4A4A] dark:text-[#FFF] border-b border-b-[#F3F3F3] dark:border-b-[#242424] font-noto">{agricultural?.date}</TableCell>
								</TableRow>
							))}
					</TableBody>
				</Table>
