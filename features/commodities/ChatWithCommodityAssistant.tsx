...
const ChatWithCommodityAssistant = ({ handleCloseChat }: { handleCloseChat?: any }) => {
    ...
    const handleSendMessage = (customMessage?: string) => {
        ...
        sendMessage(
            { messages: [{ ai: false, text: trimmed }], assetType: "commodities", userId: userData?.data?.id || "", symbol: communityId },
            {
                onSuccess: (res) => {
                    const aiMsg = {
    ...
    return (
        <div className="h-full flex flex-col bg-[var(--bg-chat)] drop-shadow-xl rounded-xl overflow-hidden">
            ...
            {/* Project Info */}
            <div className="p-4 border-b border-b-[var(--border-popover)] text-[var(--text)]">
                {isGettingCommunity ? (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-1 flex-1">
                            ...
                            <div className="flex items-center gap-2 text-xs">
                                <span className="font-medium font-noto">${communityOverview?.unit}</span>
                                <span>•</span>
            ...
            {!isFetchingSuggestions && (aiSuggestions?.suggestions ?? []).length > 0 && (
                <div className="px-4 pb-3 border-t border-[var(--border-popover)]">
                    <p className="text-xs my-2 font-reddit text-[var(--text)]">
                        Suggested questions
                    </p>
...
export default ChatWithCommodityAssistant;